...
import cn.siqishangshu.netant.LogUtil;
 ...
 */
public class PinYinDoubleChar {
    public static final String DATA_PATH = "/home/liuchuan/Documents/pinyin";
    public static final String COUNT_DATA_PATH = "/home/liuchuan/Documents/txt/double";
            ...
            if(file.isFile() && file.exists()){
                ...
                while ((lineTxt = bufferedReader.readLine()) != null ){
                    ...
                    for (int i= 1; i< chars.length; i++ ) {
                        if(charMap.containsKey(chars[i-1]+""+chars[i])){
                            charMap.put(chars[i-1]+""+chars[i],charMap.get(chars[i-1]+""+chars[i])+1);
                        }
                    }
                }
                read.close();
    ...
    public static void countCharPlusStart(){
        ...
        for (Map.Entry<String,Integer> entry:charMap.entrySet()) {
            LogUtil.writeLog(entry.getKey()+"   "+entry.getValue(),"DoubleCharCount.txt");
        }
    }
    ...
    private static void charMapInit() {
        InputStream stream = ClassLoader.getSystemResourceAsStream("new_double_ngram.freq");
